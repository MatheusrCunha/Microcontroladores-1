...
#define CHAVEBUTTON_MIN 12
#define CHAVEBUTTON_HORAS 12
...
#endif
{
    ...
    static uint8_t conta_minutos = CHAVEBUTTON_MIN;
    static uint8_t conta_horas = CHAVEBUTTON_HORAS;
    ...
    }else if(!TST_BIT(PORT_IN(RELOGIO_BUTTONS_PORT), BUTTON_MIN)){
        if (!(--conta_minutos) && meu_relogio.em_ajuste){
    ...
    }else if(!TST_BIT(PORT_IN(RELOGIO_BUTTONS_PORT), BUTTON_HORA)){
        if(!(--conta_horas) && meu_relogio.em_ajuste){
            meu_relogio.horas++;
             if(meu_relogio.minutos == 24){
                meu_relogio.horas = 0;
             }
             /* Acorda função main */
            __bic_SR_register_on_exit(LPM0_bits);
